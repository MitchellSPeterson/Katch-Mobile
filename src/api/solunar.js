import { getMoonPosition, getMoonIllumination, getTimes, getMoonTimes } from 'suncalc';
...
var getMoonTransitTimes = function(date, lat , lng) {
    ...
    for (i = 0; i <= 25; i++) {
        ...
        var moontimes = getMoonPosition(date2, lat, lng);
        if (i === 0) {
    ...
    for (j = 0; j < 60; j++) {
        ...
        date3.setSeconds(0);
        date2.setMilliseconds(0);
        var moontimes = getMoonPosition(date3, lat, lng);
...
var getMoonPhase = function(date) {
    ...
    night.setHours(24);
    var morningphase = getMoonIllumination(morning).phase,
        nightphase = getMoonIllumination(night).phase,
        phase = getMoonIllumination(date).phase;
...
export function getSolunarTimes(date, lat, lng) {
    var suntimes = getTimes(date, lat, lng),
